...
export function ModalEditBoard({closeModal} : {
    closeModal: () => void;
}) {

      const [boardName, setBoardName] = useState("Soft Vodooz")
      ...
      <div onClick={closeModal} className="modal-overlay fixed inset-0 bg-slate-800 bg-opacity-50 flex items-center justify-center">
          ...
          <div className="modal-content rounded-md fixed bg-gray-100 px-8 pt-8 pb-5 text-slate-900 w-1/4" onClick={handleModalClick}>
            ...
            <p className="">
              <form>
                <Input label="Nom du tableau" type="text" placeholder="Soft Vodooz" value={boardName} onChange={(e: Event) => setBoardName(e.target.value)} />
